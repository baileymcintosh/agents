...
from agentorg.tools.search import (
    FETCH_DOCUMENT_TOOL_DEFINITION,
...
class BaseAgent(ABC):
    ...
    def call_claude(self, user_message: str, extra_context: str = "") -> str:
        ...
        tools = [SEARCH_TOOL_DEFINITION, FETCH_URL_TOOL_DEFINITION, FETCH_DOCUMENT_TOOL_DEFINITION] if self.use_search else []
                ...
                for block in response.content:
                    if block.type == "tool_use":
                        ...
                        if tool_name == "web_search":
                            ...
                            url = tool_input.get("url", "")
                            logger.info(f"[{self.role}] Fetching document: {url}")
                            result_text = fetch_document(url)
                        else:
