...
class QuantBuilderAgent:
    ...
    def run_turn(
        ...
        prompt_parts.append(
            ...
            "5. Before every plt.show(), call set_source('Source: <dataset>') to annotate the chart "
            "   (e.g. set_source('Source: Yahoo Finance (yfinance)') or set_source('Source: FRED')). MANDATORY.\n"
            "   Then call plt.show() to save.\n"
            "   CRITICAL: ALWAYS use plt.show() — NEVER call plt.savefig() directly. plt.show() is intercepted to "
            "   auto-name and log charts properly. Direct plt.savefig() calls bypass this and produce untracked files.\n"
            "6. FORBIDDEN chart types — do NOT generate these (fabricated data, no analytical value):\n"
            "   - Scenario probability charts (e.g. 'Base Case 60%' — made-up numbers)\n"
            "   - Market impact projection charts (estimated % moves by asset class)\n"
            "   - Event timeline charts (the reporter generates an events table separately)\n"
            "   Every chart MUST draw data directly from yfinance, FRED, EIA, or another real dataset.\n"
            "7. End your response with a `## Questions for Qual` section for anything you see "
            ...
            "9. When search results point to relevant full articles, papers, or Substack posts, call `fetch_url` "
