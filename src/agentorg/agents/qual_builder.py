...
class QualBuilderAgent:
    ...
    def run_turn(
        ...
        prompt_parts.append(
            ...
            "- **CRITICAL — source linking:** Every claim's `source_ids` array MUST contain at least one "
            "source ID (e.g. 'S1') that you defined in the `sources` array above in this same JSON block. "
            "A claim with an empty `source_ids: []` will fail verification and block the entire report from "
            "being published. If you cannot find a source for a claim, either (a) lower its materiality to "
            "'supporting' and lower confidence below 0.5, or (b) omit it entirely.\n"
            "- **CRITICAL — no unsourced core claims:** Do NOT write core claims about specific "
            "companies, funds, figures, or events unless you have a real URL to cite. Generic hedged "
            "statements ('regulators are increasing scrutiny') without a specific source are not acceptable.\n"
