    ...
    def run(self, dry_run: bool = False, revision_instructions: str = "") -> dict[str, Any]:
        ...
        if quant_charts:
            ...
            chart_catalogue += (
                ...
                "  2. A `[CHART: filename]` placeholder on its own line (exactly this format)\n"
                "  3. A substantive 2-4 paragraph analysis that covers:\n"
                "     - What the data actually shows (specific numbers, trends, turning points)\n"
                "     - Why it matters to the thesis of this report\n"
                "     - How it connects to or validates the qualitative findings\n"
                "     - Any anomalies or surprises worth flagging to decision-makers\n"
        ...
        for entry in quant_charts:
            ...
            if fname and chart_file.exists():
                chart_lookup[fname] = chart_file

        # Replace every [CHART: filename] placeholder with an actual image embed
        import re as _re
        def _resolve_chart_placeholder(m: "_re.Match[str]") -> str:
            fname = m.group(1).strip()
            p = chart_lookup.get(fname)
            if p:
        ...
        unreferenced = [e for e in quant_charts if e.get("filename", "") not in referenced_fnames
                        and (config.REPORTS_DIR / e.get("filename", "")).exists()]
        if unreferenced:
